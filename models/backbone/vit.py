...
class HuggingFaceViT(nn.Module):
    ...
    def __init__(self, name='google/vit-base-patch16-224-in21k',
                 pooler_type='default', **kwargs):
        super().__init__()

        self.model = ViTModel.from_pretrained(name)  # type: ViTModel

        if pooler_type == 'linear':
            self.model.pooler.activation = nn.Identity()
        elif pooler_type == 'cls':
            self.model.pooler = Lambda(lambda x: x[:, 0, :])
        elif pooler_type == 'avg':
            self.model.pooler = Lambda(lambda x: x[:, 1:, :].mean(dim=1))
